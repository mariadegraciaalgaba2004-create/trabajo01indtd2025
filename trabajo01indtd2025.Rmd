...
X2 <- crea.tablaX(
  ...
  nb_alternativas = c("B1_Tarjeta","B2_DescuentosApp","B3_Suscripcion","B4_CampañasEco"),
  nb_estados = c("F1","F2","F3","F4")
)
X2

```
```{r}
criterio.Wald(X2, favorable=TRUE)

```
Segun el criterio de Wald, la mejor alternativa es la 4, que es la de Campañas ECO

```{r}

criterio.Optimista(X2, favorable=T)

```
Según el criterio Optimista, la mejor alternativa es la 3, que es suscripción.

```{r}

criterio.Hurwicz(X2, alfa= 0.5, favorable=T)

```
Según el criterio Optimista, la mejor alternativa es la 2 y 3, es decir DescuentosApp Suscripcion

```{r}

dibuja.criterio.Hurwicz_Intervalos(X2, favorable=T, T)

```
Para alfa con valor entre ( 0 , 0.4 ) , la mejor Alternativa es la 4.
Para alfa con valor entre ( 0.4 , 0.5 ) , la mejor Alternativa es la 2.
Para alfa con valor entre ( 0.5 , 1 ) , la mejor Alternativa es la 3.

```{r}

criterio.Savage(X2, favorable=T)
```
Segun el criterio de Savage, la mejor alternativa es la 2.

```{r}
criterio.PuntoIdeal(X2, favorable=T)
```
Segun el criterio de PuntoIdeal , la mejor alternativa es la 3.

```{r}
criterio.Laplace(X2, favorable=T)
```

Segun el criterio de LaPlace, la mejor alternativa es la 3.

```{r}
criterio.Wald(X2, favorable = F)
```
Segun el criterio de Wald, la mejor alternativa es 4.
...
X <- crea.tablaX(
  c(120,90,60,40,
    ...
    100,85,65,55),
  numalternativas = 4, numestados = 4,
  nb_alternativas = c("B1_FrutasVerduras","B2_ProductosLocales","B3_Refrigeracion","B4_ComidasFrescas"),
  nb_estados = c("F1","F2","F3","F4")
)

criterio.Todos(X, alfa=0.4, favorable=TRUE)
```
Vemos que la alternativa 3 es la Óptima en todos los criterios, es decir la Refrigeracón.
